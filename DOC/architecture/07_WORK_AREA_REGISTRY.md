...
Product direction and architecture documentation.

Allowed files:

```text
DOC/architecture/00_PRODUCT_NORTH_STAR.md
DOC/architecture/01_PRODUCT_SYSTEM_WORKFLOW.md
...
Database reference and to-be architecture documentation.
...
Dashboard summary/status display and lightweight status APIs.
...
Recent logs displayed in dashboard/admin UI.
...
Admin login, device approval, session, auth headers.
...
Any unrelated task requiring ADMIN_AUTH changes must stop.

AREA: FACEBOOK_STATUS

Purpose:

Facebook token/page status display and error classification.
...
Actual Facebook publishing behavior.
...
Any unattended task requiring FACEBOOK_PUBLISHER changes must stop.
...
News candidate normalization, duplicate classification, scoring.
...
Content candidate management and listing.
...
Automatic or manual content publishing.

Allowed changes:

Only with explicit user approval unless task is read-only audit.
...
Unattended harness must not change CONTENT_PUBLISHER behavior.

AREA: OCCUPATION_DICTIONARY

Purpose:

Employment24 occupation/job dictionary collection and display.
...
Immigration/visa official information.
...
Local LLM/Ollama status and optional helper use.
...
Schedulers, bot ON/OFF, cooldown, recurring jobs.

Allowed changes:

Only with explicit user approval unless read-only audit.
...
Operation summaries and harness reports sent to Telegram.
